...
def _matches_versioned(model, base):
    return model == base or model.startswith(base + "-")


def capabilities(provider, model=None):
    _provider_model(provider, model)
    p, m = provider.lower(), (model or "").lower()
    if p == "claude":
        if m == "haiku" or _matches_versioned(m, "claude-haiku-4-5"):
            return set()
        if m in {"sonnet", "opus"} or any(_matches_versioned(m, base) for base in
                ("claude-sonnet-4-6", "claude-opus-4-6")):
            return {"low", "medium", "high", "max"}
        if any(_matches_versioned(m, base) for base in
               ("claude-sonnet-5", "claude-opus-5", "claude-opus-4-7", "claude-opus-4-8")):
            return set(LEVELS)
    if p == "codex":
        if m == "gpt-5.5":
            return set(LEVELS[:-1])
        if m in {"gpt-5.6-luna", "gpt-6-astra"}:
            return set(LEVELS)
    return None


def route(provider, model, task="", dimensions=None, task_kind=None,
          requested=None, cap=None):
    _provider_model(provider, model)
    ...
    support = capabilities(provider, model)
