"""Graphify выборки и ограниченная группа дешёвых читателей."""
import argparse
...
from reasoning_router import validate_reasoning_config
sys.path.insert(0,str(Path(__file__).resolve().parents[1]))
from mam import project_id

KINDS={'summary':('claude','haiku'),'code':('codex','gpt-5.6-luna')}


def shared_root():
    return Path(os.environ.get('MAM_SHARED',Path.home()/'.agent-harness')).resolve()
...
def index_for(project):
    ...
    # Полный путь, а не basename: одинаковые имена проектов не смешиваются.
...
def batch(project,tasks):
    ...
    prepared=[];seen=set()
    # Проверяем ВСЕ задания до вызова моделей, затем дедуплицируем одинаковые.
    for task in tasks:
        ...
        seen.add(signature);prepared.append((kind,question,list(paths),reasoning))
    graph_paths={}
    if entry:
        for _,question,_,_ in prepared:
            if question not in graph_paths:
                graph_paths[question]=graph_context(project,question,entry)
    def work(item):
        kind,question,paths,reasoning=item
        try:
            graph_path=graph_paths.get(question)
            if graph_path: paths.append(str(graph_path))
            provider,model=KINDS[kind]
            cfg={**settings(),'provider':provider,'model':model,
            ...
            # Исходники и полная история CLI не возвращаются основному агенту.
