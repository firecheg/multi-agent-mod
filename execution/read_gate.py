"""Bound full-file reads issued through Claude's Bash and Read hooks.

This is deliberately a small recognizer, rather than a shell parser.  Commands
whose syntax is not understood are allowed through for the normal tool to
handle; the gate only denies a confidently identified unbounded read.
"""
...
def _deny(path: Path, lines: int, threshold: int) -> dict:
    return {"hookSpecificOutput": {
        ...
        "permissionDecisionReason": (
            f"Полное чтение {path.name} ({lines} строк) ограничено. "
            "Используйте поиск и диапазон либо agent_harness.context_read с конкретным вопросом "
            "(для связанных файлов — research_batch/Graphify)."
        ),
    }}
