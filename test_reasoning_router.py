...
class ReasoningTests(unittest.TestCase):
    ...
    def test_cap_is_visible_and_auto_never_max(self):
        decision = r.route('codex', 'gpt-6-astra', dimensions={name: 2 for name in r.DIMENSIONS}, cap='high')
        ...
        self.assertNotEqual(r.route('codex', 'gpt-6-astra', task_kind='incident')['effective'], 'max')
        self.assertEqual(r.route('codex', 'gpt-5.5', requested='max')['status'], 'unsupported')
        self.assertEqual(r.route('codex', 'gpt-5.6-luna', requested='max')['effective'], 'max')

    def test_real_model_capabilities_and_unknowns(self):
        self.assertEqual(r.route('claude', 'claude-haiku-4-5', task_kind='security')['status'], 'unsupported')
        self.assertEqual(r.route('claude', 'sonnet', requested='xhigh')['status'], 'unsupported')
        self.assertEqual(r.route('claude', 'opus', requested='max')['effective'], 'max')
        self.assertEqual(r.route('claude', 'claude-sonnet-5', task_kind='incident')['effective'], 'xhigh')
        self.assertEqual(r.route('claude', 'claude-opus-4-8-20260901', task_kind='incident')['effective'], 'xhigh')
        unknown = r.route('codex', None, task_kind='security')
