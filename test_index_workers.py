...
class WorkerTests(unittest.TestCase):
    ...
    def test_workers_bounded_and_models_selected(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(models,{'haiku','gpt-5.6-luna'})
    ...
    def test_codex_worker_has_no_user_config_shell_or_plugins(self):
        with patch.object(worker_cli.shutil,'which',return_value='codex'):
            args=worker_cli.command('codex','gpt-5.6-luna',Path('run'))
        self.assertIn('--ignore-user-config',args)
        self.assertIn('shell_tool',args);self.assertIn('plugins',args)
        self.assertIn('read-only',args);self.assertIn('approval_policy="never"',args)
    ...
    def test_worker_persists_actual_argv_and_reuses_decision(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            decision={'recommended':'high','selected':'medium','effective':'medium',
                      'status':'ok'}
            completed=subprocess.CompletedProcess(['worker'],0,
                json.dumps({'result':'ok','subtype':'success'}),'')
            config={'provider':'claude','model':'sonnet','timeout_seconds':10,
